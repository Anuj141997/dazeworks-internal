...
export default class GanttChartResource extends LightningElement {
  ...
  setProjects() {
    ...
    Object.keys(self._resource.allocationsByProject).forEach(projectId => {
      ...
      self._resource.allocationsByProject[projectId].forEach(allocation => {
        allocation.class = this.calcClass(allocation);
        allocation.style = this.calcStyle(allocation);
        allocation.labelStyle = this.calcLabelStyle(allocation);
        allocation.title = allocation.projectName + ' (Utilisation - '+ allocation.Utilisation__c + '% , Billability - '+ allocation.Billablity__c+'%)';
  ...
  handleDragEnd(event) {
    ...
    let startDate = new Date(allocation.Start_Date__c + "T05:30:00");
    let endDate = new Date(allocation.End_Date__c + "T05:30:00");
  ...
  handleModalEditClick(event) {
    this.editAllocationData = {
      ...
      utilisation : this.menuData.allocation.Utilisation__c,
      billingRate : this.menuData.allocation.Billing_Rate__c,
      billablity : this.menuData.allocation.Billablity__c,
  ...
  editAllocationModalSuccess() {
    const startDate = new Date(this.editAllocationData.startDate + "T05:30:00");
    const endDate = new Date(this.editAllocationData.endDate + "T05:30:00");
    ...
    this._saveAllocation({
      ...
      utilisation : this.editAllocationData.utilisation,
      billablity : this.editAllocationData.billablity,
      billingRate : this.editAllocationData.billingRate
    })
      .then(() => {
        this.editAllocationData = {};
